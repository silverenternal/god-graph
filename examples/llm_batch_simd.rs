...
fn main() {
    ...
    use god_gragh::transformer::{
        MultiHeadAttention, RMSNorm, FeedForward,
        perf::{TransformerMemoryPool, matmul_with_buffer, softmax_inplace_simd},
    };
    ...
    println!("   Time ({} iters): {:.2} ms", iterations, elapsed.as_secs_f64() * 1000.0);
    ...
    let elapsed = start.elapsed();
    println!("   Time ({} iters): {:.2} ms", iterations, elapsed.as_secs_f64() * 1000.0);
    println!("   Time per iter: {:.2} ms", elapsed.as_secs_f64() * 1000.0 / iterations as f64);
    ...
    println!("   Traditional allocation: {:.2} ms", elapsed_traditional.as_secs_f64() * 1000.0);
    println!("   Memory pool allocation: {:.2} ms", elapsed_pool.as_secs_f64() * 1000.0);
