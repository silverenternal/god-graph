...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for step in 0..num_steps {
        ...
        if step % 10 == 0 {
            ...
            let current_edges = diff_graph.edges()
                .filter(|(_, edge)| edge.exists)
                .count();
    ...
    diff_graph.discretize();
    let final_edge_count = diff_graph.edges()
        .filter(|(_, edge)| edge.exists)
        .count();
...
fn run_synthetic_demo() {
    ...
    for step in 0..30 {
        ...
        if step % 10 == 0 {
            ...
            // 打印不同重要性边的概率
            let local_edge_prob = diff_graph.edges()
                .filter(|(&(s, d), _)| (s as i32 - d as i32).abs() <= 1)
                .map(|(_, e)| e.probability)
                .next()
                .unwrap_or(0.0);
            let far_edge_prob = diff_graph.edges()
            ...
            println!(
                ...
                step, diff_graph.config().temperature, current_edges, local_edge_prob, far_edge_prob
            );
        }
    ...
    // 最终离散化
    diff_graph.discretize();
    let final_edges = diff_graph.edges()
        .filter(|(_, edge)| edge.exists)
        .count();
    ...
    // 分类统计
    let very_near_edges = diff_graph.edges()
        .filter(|(&(s, d), edge)| (s as i32 - d as i32).abs() <= 1 && edge.exists)
        .count();
    let near_edges = diff_graph.edges()
        .filter(|(&(s, d), edge)| (s as i32 - d as i32).abs() == 2 && edge.exists)
        .count();
    let far_edges = diff_graph.edges()
        .filter(|(&(s, d), edge)| (s as i32 - d as i32).abs() > 2 && edge.exists)
        .count();
    
    println!("\n优化完成:");
    println!("  最终边数：{}/{} ({:.2}%)", final_edges, total_possible, (final_edges as f64 / total_possible as f64) * 100.0);
    println!("  非常近边 (距离≤1): {} 条保留", very_near_edges);
    println!("  较近边 (距离=2): {} 条保留", near_edges);
    println!("  远距离边 (距离>2): {} 条保留", far_edges);
    println!("  剪枝比例：{:.2}%", (total_edges - final_edges) as f64 / total_edges as f64 * 100.0);
}

#[cfg(not(all(feature = "tensor", feature = "safetensors")))]
fn main() {
    println!("请启用 tensor 和 safetensors 特性运行此示例:");
    println!("  cargo run --example differentiable_attention_pruning --features tensor,safetensors");
}
