...
fn bench_arena_vs_pool_reuse(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        // TensorPool 复用
        group.bench_with_input(
            BenchmarkId::new("pool_reuse", size),
            &shape,
            |b, s| {
                let config = PoolConfig::new(16, 64);
                let mut pool = TensorPool::new(config);
        ...
        // TensorArena 复用
        group.bench_with_input(
            BenchmarkId::new("arena_reuse", size),
            &shape,
            |b, s| {
                let mut arena = TensorArena::with_capacity(16 * 1024 * 1024);
                b.iter(|| {
...
fn bench_arena_shape_aware_reuse(c: &mut Criterion) {
    ...
    for shape in shapes {
        ...
        // 场景 2: 不同形状交替分配 (无复用)
        let shape2 = if size < 64 { vec![size * 2, size * 2] } else { vec![size / 2, size / 2] };
        group.bench_with_input(
...
fn bench_arena_statistics(c: &mut Criterion) {
    ...
    for shape in shapes {
        ...
        group.bench_with_input(
            BenchmarkId::new("stats_overhead", size),
            &shape,
            |b, s| {
                let mut arena = TensorArena::with_capacity(16 * 1024 * 1024);
                b.iter(|| {
