...
```rust
use god_graph::tensor::differentiable::ThresholdEditPolicy;
...
```rust
use god_graph::tensor::differentiable::{GradientEditPolicy, StructureEdit};
...
```rust
use god_graph::tensor::differentiable::{EdgeEditPolicy, StructureEdit};
...
fn build_attention_graph(
    ...
    n_heads: usize,
    seq_len: usize,
) -> god_graph::graph::Graph<Vec<f64>, f64> {
    use god_graph::graph::traits::GraphOps;
...
fn add_candidate_residual_connections(
    graph: &mut god_graph::graph::Graph<Vec<f64>, f64>,
    n_layers: usize,
) {
    // 添加所有可能的跳跃连接（后续可剪枝）
    for i in 0..n_layers {
